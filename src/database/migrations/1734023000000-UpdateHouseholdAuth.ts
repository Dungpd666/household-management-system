...
export class UpdateHouseholdAuth1734023000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Thêm cột is_active
    await queryRunner.query(
      `ALTER TABLE "households" ADD "is_active" boolean NOT NULL DEFAULT true`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "households" RENAME COLUMN "password" TO "password_hash"`,
    );

    // Thêm lại cột username
    await queryRunner.query(
      `ALTER TABLE "households" ADD "username" character varying`,
    );
    await queryRunner.query(
      `ALTER TABLE "households" ADD CONSTRAINT "UQ_households_username" UNIQUE ("username")`,
    );
  }
}
