...
export const PopulationOverviewCard = ({ data }: { data: PopulationOverviewData }) => {
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-[1fr,120px] gap-3 items-center">
            ...
            return (
              <div key={item.key} className="flex items-center justify-between gap-2 text-sm">
                <div className="flex items-center gap-2">
                  <span className={`inline-block h-2.5 w-2.5 rounded-full ${item.className}`} style={{ backgroundColor: 'currentColor' }} />
                  <span className="text-textc-secondary">{item.name}</span>
                </div>
                <div className="text-right">
                  <div className="font-semibold text-textc-primary">{item.value.toLocaleString()}</div>
                  <div className="text-[12px] text-textc-faint">{formatPercent(percent)}</div>
                </div>
              </div>
            );
          })}
